...
const CompanionsList = ({title, companions, classNames }:CompanionsListProps) => {
  return (
    <article className={cn('companion-list', classNames)}>
      <h2 className="font-bold text-3xl ">Recent Tables</h2>
      <Table>
        <TableHeader>
          <TableRow>
          ...
          {companions?.map(({id,subject, name, topic, duration}) => (
            <TableRow key={id}>
              ...
              <TableCell>
                ...
                <div className="w-fit p-2  flex items-center justify-center rounded-lg md:hidden" style={{backgroundColor: getSubjectColor(subject)}} >
                  <Image src={`/icons/${subject}.svg`} alt="subject" width={18} height={18} />
                </div>
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-2 justify-end w-full ">
                  <p className="text-lg">{duration} {' '}<span className="max-md:hidden">mins</span></p>
